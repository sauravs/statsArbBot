...
- **Operator approval is the gate — do NOT skip it.** Never merge to `main`, and never promote `main → production`, without the operator's explicit OK. Promotion is always `main → production` (never commit straight to `production`; never let it get ahead of `main`). Deploy to the server only after approval: `cd ~/statsArbBot && git pull && docker compose up -d` (on the `production` branch). Live deployment facts: `docs/DEPLOYMENT.md` §0.
