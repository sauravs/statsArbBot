...
**Per-trade cost breakdown (the blotter).** Expand a per-window row (marked ▸) to get the **trade blotter**, which shows every closed trade and — as of Phase-4 — *why* each one landed where it did: **Hold · Gross · Fees · Funding · Net P&L**, with the identity **Gross + Fees + Funding = Net** printed above the table. **Funding** is the one to watch: it is signed (a long leg pays, a short leg receives) and it **accrues with hold time**, so read it against the **Hold** column — a trade held 3× longer pays roughly 3× the funding, which is exactly the drag that turns a reverted (take-profit) spread into a net loss. Use the **Losing take-profits** toggle to isolate that cohort. **Slippage and market impact are deliberately not their own column**: the engine charges them at the *fill price* (`backend/simulation/costs.py::apply_slippage`, `simulation/market_impact.py`), so they are already inside **Gross** — which therefore is **not** a mid-price figure. A ⚠ beside a Net value means that row's components don't add up to its stored net (a data defect, not a rounding artefact).

**Cost decomposition (the aggregate view).** The same breakdown, summed: a **whole-run** panel sits under the headline metrics, and each window's **own** panel appears at the top of its blotter when you expand it. Both show `Gross / Fees / Funding / Net` plus trade count and **average hold**, and both summarise *every* trade in scope — not just the 25 rows on the current blotter page. This is the fastest way to read a run: e.g. *gross $29.38, fees −$27.09, funding $0.00 → net $2.29 over 149 trades* says "the signal worked and the fees took almost all of it" at a glance, which is exactly the diagnosis a campaign's results need. Served by `GET /api/backtest/strategies/{id}/costs`, aggregated in Postgres from the persisted trades — so it works **retroactively for every saved run** (a run that predates the per-trade blotter simply has no trade rows and shows nothing).

**When the strategy list comes back empty.** The list shows `0 of N shown` and names the filter responsible (e.g. *"The Phase filter is set to 'Phase 2', and all 75 saved runs are Phase 1"*), with a **Reset filters** button. All five filters default to "everything" — in particular **Phase defaults to All**, so a phase-1-only list is never hidden unless you asked for it.
